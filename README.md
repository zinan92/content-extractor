...
**多模态内容提取器 — 把下载的视频/图片/文章/图集变成结构化文本 + 智能分析**
...
version: 0.1.0
capability:
  summary: "Extract structured text and analysis from multimedia content (video/image/article/gallery)"
  in: "ContentItem directory (content-downloader output with content_item.json + media files)"
  out: "transcript.json + analysis.json + structured_text.md"
  fail:
...
cli_args:
  - name: path
    type: string
    required: true
    description: "Path to ContentItem directory or parent directory"
cli_flags:
  - name: --whisper-model
    type: string
